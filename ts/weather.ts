...
const API_KEY: string = 'ffe6804c8a87c4ef8e83eb850ea4fd5d';
const diffKelvin: number = 273.15;
const weatherBox: HTMLElement = document.getElementById('weatherBox') as HTMLElement;

const getWeatherData = async (api: string, key: string): Promise<any> => {
	const response = await fetch(api+'?q=Barcelona&appid='+key).then(response => response.json());
	return response;
}

const processWeatherData = async (): Promise<void> => {
	try {
